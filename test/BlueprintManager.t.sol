...
contract BlueprintManagerTest is Test {
    ...
    function test_creditDebitRevertWithoutCool(
        address user,
        uint256 amount
    ) external {
        ...
        calls[0] = BlueprintCall(
            ...
            postChecksum
                ? keccak256(
                    abi.encodePacked(
                        NULL_HASH,
                        keccak256(abi.encodePacked(uint256(0), amount)),
                        NULL_HASH,
                        NULL_HASH
                    )
                )
    ...
    // function test_vesting(address from, uint256 amount, uint256 batch, uint256 measureTimestamp, bool noSchedule)
    //     public
    // {
    //     // Basic validation
    //     vm.assume(from != address(vesting));

    //     // Constrain amount and batch to reasonable ranges
    //     amount = bound(amount, 1, 1000000); // Limit amount to reasonable range
    //     batch = bound(batch, 2, 100); // Limit batch to reasonable range

    //     // Ensure total amount calculation won't overflow
    //     vm.assume((amount * batch) / batch == amount);

    //     // Time constraints
    //     uint256 baseTime = block.timestamp;
    //     measureTimestamp = bound(
    //         measureTimestamp,
    //         baseTime + 1000, // Start time
    //         baseTime + 3000 // End time
    //     );
    ...
    //     console2.log("Total amount:", amount * batch);
    //     console2.log("Start time:", start);
    //     console2.log("Measure time:", measureTimestamp);
    ...
    //             from,
    //             address(vesting),
    //             abi.encode(id, amount * batch, 0, address(schedule), params, batch, batch / 2),
    //             bytes32(0)
    //         )
    //     );
    ...
    //         (vestingId, balance) = getLinearCliffVestingPosition(id, amount * batch, amount * (batch / 2), params);
    ...
    //             from,
    //             address(vesting),
    //             abi.encode(id, amount * batch, amount * (batch / 2), address(schedule), params, batch, batch),
    //             bytes32(0)
    //         )
    //     );
    ...
    //     (vestingId, balance) = getLinearCliffVestingPosition(id, amount * batch, amount * batch, params);
    ...
    //     redeemMaxLinearCliffVesting(from, id, amount * batch, amount * batch, params);
    ...
    //         id, amount * batch, amount * batch - schedule.getVestedTokens(amount * batch, params), params
    //     );
    ...
    //             ? 0
    //             : (measureTimestamp > end ? amount * batch : (amount * batch * (measureTimestamp - start)) / 1000)
    //     );
    // }

    function test_erc20Wrap(
        address from,
        address to,
        uint256 amount
    ) public returns (uint256 id) {
        ...
        id = HashLib.getTokenId(
            address(erc20wrapper),
            uint256(uint160(address(erc20)))
        );
    ...
    function test_basket(
        ...
        uint256 id = HashLib.getTokenId(
            address(basket),
            uint256(keccak256(abi.encodePacked(ids, amounts)))
        );
    ...
    function test_transferFrom(
        ...
        vm.startPrank(from);
        if (setApproval) manager.approve(sender, id, amount);
        ...
        vm.prank(sender);
        if (willFail) vm.expectRevert();
        manager.transferFrom(from, address(this), id, amount);

        if (!willFail) assertEq(manager.balanceOf(address(this), id), amount);
    }
}
